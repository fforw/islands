...
var CLIFF_THRESHOLD = 8;
var RANDOM_FACTOR = 0; // size of the outer square around our big hexagon
...
var STONE = 5;
var STONE2 = 6;
var GROUND_COLORS = (_GROUND_COLORS = {}, _defineProperty(_GROUND_COLORS, WATER, [0, 0.4, 0.8]), _defineProperty(_GROUND_COLORS, SAND, [0.8, 0.8, 0]), _defineProperty(_GROUND_COLORS, GRASS, [0, 0.5, 0]), _defineProperty(_GROUND_COLORS, DIRT, [0.5, 0.3, 0.1]), _defineProperty(_GROUND_COLORS, FOREST, [0.2, 0.4, 0.3]), _defineProperty(_GROUND_COLORS, STONE, [0.5, 0.5, 0.5]), _defineProperty(_GROUND_COLORS, STONE2, [0.6, 0.6, 0.6]), _GROUND_COLORS);
var container, stats;
...
function cutCliffs() {
  ...
  for (var i = 0; i < length; i += _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_size"]) {
    ...
    if (cutMask !== 0) {
      for (var j = 0; j < 4; j++) {
        if (cutMask & 1 << j) {
          var heightMapIndex0 = tiles[i + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_n0"] + j] * heightIndexFactor;
          var heightMapIndex1 = (j === 3 ? tiles[i + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_n0"]] : tiles[i + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_n0"] + j + 1]) * heightIndexFactor;
          var cut0 = td_cut0 + j;
          var cut1 = j === 3 ? td_cut0 : td_cut0 + j + 1;
          var edgeBefore = j === 0 ? 3 : j - 1;
          var edgeAfter = j === 3 ? 0 : j + 1;
          var heightOnQuadBefore = tmpHeight[1 + edgeBefore];
          var heightOnQuadAfter = tmpHeight[1 + edgeAfter];
          var height0 = heightOnQuadBefore >= 0 ? (heightOnQuadBefore + tmpHeight[0]) / 2 : tmpHeight[0];
          var height1 = heightOnQuadAfter >= 0 ? (tmpHeight[0] + heightOnQuadAfter) / 2 : tmpHeight[0];
          tileData[tileDataIndex + cut0] = height0;
          tileData[tileDataIndex + cut1] = height1;
          heightMap[heightMapIndex0 + h_ground] = STONE;
          heightMap[heightMapIndex1 + h_ground] = STONE; // cut our connection to the other tile

          tiles[i + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_tile0"] + j] = -1;
        }
      }
    }
  }
...
function walkRecursive(tileIndex, visited) {
  if (tileIndex >= 0 && !visited.has(tileIndex)) {
    ...
    walkRecursive(tiles[tileIndex + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_tile0"]], visited);
    walkRecursive(tiles[tileIndex + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_tile1"]], visited);
    walkRecursive(tiles[tileIndex + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_tile2"]], visited);
    walkRecursive(tiles[tileIndex + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_tile3"]], visited);
  }
}
...
function createScene() {
  ...
  var length = graph.length;
  var heightMapFactor = h_size / _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["g_size"];
  ...
  for (var _i = 0; _i < tiles.length; _i += _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_size"]) {
    if (tiles[_i + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_isEdge"]]) {
      for (var j = 0; j < 4; j++) {
        var node = tiles[_i + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_n0"] + j];
        var hIdx = node * heightMapFactor;
...
function addHeightMap() {
  ...
  var getColor = function getColor(hIdx) {
    var tileDataIndex = hIdx * td_size / h_size;
    var ground = heightMap[hIdx + h_ground];
    var color = GROUND_COLORS[ground];
  ...
  for (var i = 0; i < length; i += _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_size"]) {
    ...
    var walkable = tileData[tileDataIndex + td_walkable];
  ...
  mesh.position.set(0, 0.2, 0); // var wireframe = new WireframeGeometry( geometry );
  //
...
/******/ });
//# sourceMappingURL=bundle-main-c0b42b03909b2034bd55.js.map