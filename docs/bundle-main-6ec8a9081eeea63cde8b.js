...
function cutCliffs() {
  ...
  for (var i = 0; i < length; i += _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_size"]) {
    ...
    if (cutMask !== 0) {
      // we take over the height of our centroid
      var height = tmpHeight[0];
      ...
      for (var j = 0; j < 4; j++) {
        if (cutMask & 1 << j) {
          ...
          tileData[tileDataIndex + cut0] = height;
          tileData[tileDataIndex + cut1] = height;
...
function walkRecursive(tileIndex, visited) {
  if (tileIndex >= 0 && !visited.has(tileIndex)) {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (!tiles[tileIndex + _fforw_organic_quads__WEBPACK_IMPORTED_MODULE_5__["t_isEdge"]]) {
...
/******/ });
//# sourceMappingURL=bundle-main-6ec8a9081eeea63cde8b.js.map